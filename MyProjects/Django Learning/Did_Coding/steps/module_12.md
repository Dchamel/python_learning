...
Okay lets start where we left off in the last module. We have built our database tables and spent some time creating an
API manager. In this module, we will create the following views.
...
2) URL's - Like always, when we adjust our views we should then make sure we check our url's. In this case we need to
   create a new file called urls.py in /ecommerce. Create the file and user the following code.
